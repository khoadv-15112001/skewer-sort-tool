...
namespace SonatUI.PageSlider
{
    ...
    public class PageSlider : MonoBehaviour
    {
        ...
        /// </summary>
        public event Action<PageContainer> OnPageChanged;
        public static event Action<int> OnPageVisible;

        /// <summary>
        /// Gets the rectangle of the PageSlider component.
        /// </summary>
        public Rect Rect { get { return ((RectTransform)transform).rect; } }
        public List<PageContainer> Pages => _pages;

        public int CurrentPage { get => _scroller.CurrentPage; internal set => _scroller.SetPage(value); }
        ...
        private void Awake()
        {
            ...
            if (fullScreenWidth)
            {
                OnClickChangeFullScreenWidth();
            }
        }
        private void OnClickChangeFullScreenWidth()
        {
            //Debug.Log("_scroller: " + _scroller.Rect.size);
        ...
        {
            // Validate indices before accessing the list
            if (_pages == null || fromIndex < 0 || fromIndex >= _pages.Count || toIndex < 0 || toIndex >= _pages.Count)
            {
                Debug.LogWarning($"PageSlider: Invalid page index. fromIndex: {fromIndex}, toIndex: {toIndex}, Pages count: {(_pages?.Count ?? 0)}");
                return;
            }
        ...
        internal void RemovePage(PageContainer page)
        {
        ...
        public class PageControllerEditor : Editor
        {
            ...
            public override void OnInspectorGUI()
            {
                ...
                if (GUILayout.Button("OnClickChangeFullScreenWidth"))
                {
                    _target.OnClickChangeFullScreenWidth();
                }
